...
async function handleUserDeleted(data: any) {
  ...
  if (stripe && privateMetadata?.stripeCustomerId) {
    await stripe.customers.del(privateMetadata.stripeCustomerId);
  }
  ...
  if (
    ...
    try {
      ...
      const { error } = await archiveCustomer(privateMetadata.lemonCustomerId);
  ...
  console.log(
    `🔴 User deleted: ${id} | Stripe ID: ${user.stripeCustomerId || "Not configured"} | LemonSqueezy ID: ${user.lemonsqueezyCustomerId || "Not configured"}`
  );
}
